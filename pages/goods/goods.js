...
Page({
  ...
  getGoodsInfo: function () {
    let that = this;
    // id: that.data.id
    util.request(api.GoodsDetail, { id: 1181000 }).then(function (res) {
      if (res.errno === 0) {
        that.setData({
          goods: res.data.info,
  ...
  getGoodsRelated: function () {
    let that = this;
    // id: that.data.id
    util.request(api.GoodsRelated, { id: 1181000 }).then(function (res) {
      if (res.errno === 0) {
        that.setData({
  ...
  clickSkuValue: function (event) {
    ...
    if(_specificationList && _specificationList.length == 0 ){
        return;
    }
    for (let i = 0; i < _specificationList.length; i++) {
      if (_specificationList[i].specification_id == specNameId) {
        let list = _specificationList[i].list;
        for (let j = 0; j < list.length; j++) {
          if (list[j].id == specValueId) {
            //如果已经选中，则反选
  ...
  onLoad: function (options) {
    ...
    util.request(api.CartGoodsCount).then(function (res) {
      if (res.errno === 0) {
        that.setData({
          cartGoodsCount: res.data.cartNum
        });
        ...
        .then(function (res) {
          ...
          if (_res.errno == 0) {
            ...
            that.setData({
              ...
              cartGoodsCount: _res.data.cartTotal.cartNum
            });
          } else {
            wx.showToast({
              image: '/static/images/icon_error.png',
